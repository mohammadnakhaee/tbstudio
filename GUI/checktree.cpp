...
wxTreeItemId wxCheckTree::tree_add(const wxTreeItemId& parent, const wxString& text, bool set_state, bool checked, bool enabled )
{
    wxTreeItemId id=AppendItem(parent,text);

    if(set_state)
    {
        MakeCheckable(id, checked);
...
wxTreeItemId wxCheckTree::tree_add(const wxTreeItemId& parent, const wxString& text, int tree_node, bool set_state, bool checked, bool enabled )
{
    wxTreeItemId id=AppendItem(parent,text);
    m_items.insert(std::pair<int,wxTreeItemId>(tree_node,id));
...
wxTreeItemId wxCheckTree::FindItemNamed(const wxString &name)
{
	...
	while (!items.empty())
	{
		...
		items.pop();
        wxString ttt=GetItemText(next);
		if (next != GetRootItem() && GetItemText(next) == name)
        {
			return next;
        }
...
wxTreeItemId wxCheckTree::FindItemViaDIR(const wxString& dir)
{
    wxTreeItemId mypar = GetRootItem();
    wxStringTokenizer tokenizer(dir, "/");
    wxString childname = tokenizer.GetNextToken();
    childname = tokenizer.GetNextToken();
    while ( tokenizer.HasMoreTokens() )
    {
        childname = tokenizer.GetNextToken();
        mypar = FindItemIn(mypar, childname);
    }
    return mypar;
}

wxString wxCheckTree::MyDIR(wxTreeItemId root, wxTreeItemId ID)
{
    if (root == ID) return _("");
    wxTreeItemId myparID = GetItemParent(ID);
    wxString myText = GetItemText(ID);
    wxString myparText = GetItemText(myparID);
    return MyDIR(root, myparID) + _("/") + myparText;
}

int wxCheckTree::GetItemCount()
{
    int cnt=0;
	std::stack<wxTreeItemId> items;
	if (GetRootItem().IsOk())
